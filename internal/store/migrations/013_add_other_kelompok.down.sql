-- Remove the Other kelompok option. This intentionally fails while any
-- student is still assigned to Other; reclassify those students before
-- attempting a downgrade so no data is silently changed.

CREATE TEMP TABLE attendances_backup_013_down AS SELECT * FROM attendances;
...
ALTER TABLE students RENAME TO students_old_013_down;
...
SELECT id, name, nickname, date_of_birth, gender, level, kelompok, city,
       joined_at, left_at, leave_reason, status,
       parent_name, parent_phone, parent_email, created_at, updated_at
  FROM students_old_013_down;

DROP TABLE students_old_013_down;
...
SELECT id, date, duration_min, teacher_id, student_id, status, materi,
       created_at, updated_at, submitted_phone
  FROM attendances_backup_013_down;

DROP TABLE attendances_backup_013_down;
