...
function GenderCard({ buckets }: { buckets: Bucket[] }) {
  ...
  const genderLabel = (label: string) =>
    label === 'male' ? t('dashboard.male') : label === 'female' ? t('dashboard.female') : label
  const data = buckets
    .filter((b) => b.count > 0)
    .map((b) => ({
      name: genderLabel(b.label),
      key: b.label,
      value: b.count,
    }))
        ...
        <ul className="space-y-1.5 text-sm">
          {buckets.map((b) => (
            <li key={b.label} className="flex items-center gap-2">
              ...
              />
              <span className="font-medium">{genderLabel(b.label)}</span>
